...
- Gemini requests use system instructions, JSON Schema, typed errors, bounded retry, automatic primary-model `429` wait/retry, image fallback through Gemini 3.1 Pro Preview then Gemini 2.5 Flash for transient provider/model failures, one-at-a-time execution, request coalescing, cache, and local cooldown.
...
- Codemagic simulator workflow runs 21 unit tests covering AI, persistence, backup, validation, rate-limit wait/retry, and Gemini fallback behavior.
