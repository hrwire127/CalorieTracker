...
- Gemini requests use system instructions, JSON Schema, typed errors, bounded retry, automatic primary-model `429` wait/retry, image fallback to Gemini 2.5 Flash for transient provider/model failures, one-at-a-time execution, request coalescing, cache, local cooldown, and detailed progress messages.
...
- Codemagic simulator workflow runs 20 unit tests covering AI, persistence, backup, validation, rate-limit wait/retry, progress reporting, and Gemini fallback behavior.
